import React from "react";

function Sidebar({ isOpen, toggleSidebar }) {
    return (
        <>
            <div
                className={`fixed inset-0 bg-black bg-opacity-50 z-40 ${isOpen ? 'block' : 'hidden'} md:hidden`}
                onClick={toggleSidebar}
            ></div>

            <div
                className={`fixed top-0 left-0 z-50 bg-green-600 p-4 shadow-md h-full transition-transform transform ${isOpen ? 'translate-x-0' : '-translate-x-full'
                    } md:translate-x-0 md:w-2/12 lg:w-2/12 xl:w-2/12`}
            >

                <div className="flex justify-between items-center">
                    <h2 className="lg:text-[25px] font-semibold text-white">RecicleJá</h2>
                    <button
                        className="p-2 text-right md:hidden"
                        onClick={toggleSidebar}
                    >
                        ✖️
                    </button>
                </div>



                <ul className="mt-4 space-y-2">
                    <li className="p-2 rounded hover:bg-gray-100  hover:text-slate-950 cursor-pointer text-gray-100 ">Ecopontos</li>
                    <li className="p-2 rounded hover:bg-gray-100  hover:text-slate-950  cursor-pointer text-gray-100">Sair</li>
                </ul>
            </div>
        </>
    );
}

export default Sidebar;
